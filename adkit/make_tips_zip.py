...
def main() -> None:
    ...
    for lid, slug in LEAGUES:
        ...
        tips_table.append(f"{'Spiel':<38}{'TippsArena':<12}{'LuxTipps':<12}Quelle")
        for f in d["fixtures"]:
            pair = f"{f['home_short']} - {f['away_short']}"
            t = f"{f['tip']['home']}:{f['tip']['away']}"
            a = f"{f['alt']['home']}:{f['alt']['away']}"
            src = "Exact-Score-Markt" if f["method"] == "exact_score_market" \
                else "1X2 + Over/Under"
            tips_table.append(f"{pair:<38}{t:<12}{a:<12}{src}")
...
schicke dir keine fremde Musik mit ins Werbekonto.
  ...
  tippsarena/   6 Videos, orange, @TippsArenaMoneyrace_bot
  luxtipps/     6 Videos, gold/schwarz, @LuxTippsBot
...
wird ueber alle Buchmacher gemittelt, die den Markt stellen.
  ...
  TippsArena zeigt das wahrscheinlichste Ergebnis.
  LuxTipps zeigt das zweitwahrscheinlichste.

So sind die Ergebnisse der beiden Marken unterschiedlich, wie du es wolltest,
ohne dass eine Zahl erfunden ist. Beide stehen im Markt.
